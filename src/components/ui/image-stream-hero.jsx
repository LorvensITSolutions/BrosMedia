...
/* Corridor geometry — see component history for path notes. */
        ...
        <div className="absolute inset-0" style={{ transformStyle: 'preserve-3d' }}>
          {[right, left].map((name) =>
            Array.from({ length: cards }, (_, i) => {
              // Both rails walk the same sequence so left mirrors right at every depth.
              const img = images[i % Math.max(images.length, 1)]
              return (
                <button
                  key={`${name}-${i}`}
                  type="button"
                  disabled={!img || !onImageClick}
                  ...
                  className={cn(
                    ...
                    onImageClick
                      ? 'pointer-events-auto cursor-default transition-[filter] hover:brightness-105'
                      : 'pointer-events-none',
                  )}
                  style={{
                    left: '50%',
